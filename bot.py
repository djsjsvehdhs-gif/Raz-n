...
import playwright_stealth  # Importación corregida para evitar ImportError
from telebot.async_telebot import AsyncTeleBot

# --- CONFIGURACIÓN ---
# Asegúrate de tener la variable TELEGRAM_TOKEN en Settings > Variables de Railway
...
async def bypass_logic(url, site_type):
    async with async_playwright() as p:
        ...
        # Aplicar modo sigilo para saltar protecciones (Corregido)
        await playwright_stealth.stealth_async(page) 

        # Bloqueo de multimedia (Permitiendo scripts/estilos para que AdGate funcione)
        ...
        try:
            ...
            if site_type == "nabzclan":
                # 1. Clic en Start Now (según tu captura de pantalla)
                try:
                    start_btn = await page.wait_for_selector("text=/Start Now/i", timeout=15000)
                    if start_btn: await start_btn.click()
                except:
                    pass # Si ya está en la siguiente pantalla, continuamos
                
                # 2. Ciclo de los 3 botones Step 1, 2, 3 (Ad Gate)
                for i in range(1, 4):
                    step_btn = await page.wait_for_selector(f"text=/Step {i}/i", timeout=15000)
                    if step_btn:
                        await step_btn.click()
                        # Espera del contador de 10s + pequeño margen para el servidor
